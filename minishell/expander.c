...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/08/18 23:06:31 by mertcaki          #+#    #+#             */
/*   Updated: 2024/08/21 05:52:41 by mertcaki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	expand_dollar_variable(t_shell *shell, t_list *lex, char **temp,
		char *before) // ÖĞRENİLDİ
{
	char	*new_value;

	if (ft_isdigit((*temp)[1]))
	{
		new_value = ft_strdup(*temp + 1+ digit_count(*temp + 1));
...
static void	expand_tilde(t_shell *shell, t_list *lex) // ÖĞRENİLDİ
{	// Bu fonksiyon kullanıcının ~ karakterini gördüğü yeri HOME env değişkeni ile değiştirir
	char	*tmp;
	char	*home;
...
void	expand_dollar(t_shell *shell, t_list *lex, char **temp, char *before) // ÖĞRENİLDİ
{
	if ((*temp)[1] == '?')
