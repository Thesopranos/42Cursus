...
void	free_utils2(t_shell **shell, char *back, t_list *lex, char *before)
{
	if (!lex->content)
	{
		free(back);
...
void	free_utils3(t_shell **shell, char *before, char *new_value, t_list *lex)
{
	if (!new_value)
	{
		free(before);
...
void	free_utils4(t_shell **shell, char *before, char *new_value, t_list *lex)
{
	if (!lex->content)
	{
		free(before);
...
void	export_utils(t_shell **shell, char **key, char **value, char *text)
{
	if (ft_strchr(text, '=') != 0)
	{
